...
async def list_saved_queries(project_id: str, x_user_id: str = Header(None)):
    ...
    queries = supabase_request("GET", f"saved_queries?project_id=eq.{project_id}&order=created_at.desc")
    return queries or []
...
async def save_query(request: SaveQueryRequest, x_user_id: str = Header(None)):
    ...
    existing = supabase_request("GET", f"saved_queries?project_id=eq.{request.project_id}&name=eq.{request.name}")
    ...
    if existing:
        ...
        updated = supabase_request("PATCH", f"saved_queries?id=eq.{query_id}", body=query_payload, headers=headers)
        if not updated:
...
async def delete_query(query_id: str, x_user_id: str = Header(None)):
    ...
    query = supabase_request("GET", f"saved_queries?id=eq.{query_id}")
    if not query:
    ...
    deleted = supabase_request("DELETE", f"saved_queries?id=eq.{query_id}")
